...
# ---------------------[ FIND FILE IN DB ]---------------------#
    ...
    async def get_file_by_fileuniqueid(self, id, file_unique_id, many=False):
        """
        Find a file in DB by its file_unique_id.

        Args:
            id (int): The user id.
            file_unique_id (str): The unique id of the file.
            many (bool): If True, return all files with this unique id.
                            If False, return only the first one.

        Returns:
            A file_info document if found, or False if not found.
        """
        if many:
